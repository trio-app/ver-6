Ext.define('SystemAsset.Tassetdata.controller.C_tassetdata', {
    extend  : 'Ext.app.Controller',
    init: function () {
      this.control({
            'FRM_tassetdata button[action=add]': {
    ...
    onRowdblclick: function(me, record, item, index){
        var form = Ext.getCmp('FRM_tassetdata').down('form');
    ...
    doSaveform: function(){
        ...
        var recValue = Ext.create('SystemAsset.Tassetdata.model.M_tassetdata', values);
        console.log(action);
        if(action == 'edit') {
    ...
    removeitem: function(record){
        Ext.Msg.confirm('Delete Data.', 'Delete <b>' + record.data.AssetName + '</b>.<br/>Are you sure?', function (button) {
            if (button == 'yes') {
    ...
    doProsesCRUD : function (inAction,record){
        var grid = Ext.getCmp('GRID_tassetdata');
        var form = Ext.getCmp('FRM_tassetdata');
        var store = grid.getStore();//Ext.getStore('SystemAsset.Assetpic.store.STtassetdata');;//
        console.log(record.data);
        Ext.Ajax.request({
                    url: base_url + 'Tassetdata/' +  inAction,
                    method: 'POST',
                    //type:'json',
