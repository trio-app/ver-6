...
class R_cpuser extends CI_Model {
...
function load_default($start,$limit,$filter){
    ...
    //print_r($keyfilter);
    $this->load->database();
    $this->db->select(' SQL_CALC_FOUND_ROWS cpuser.*
                    ',FALSE);
    $this->db->from('cpuser');
    $this->db->where("cpuser.userID <>", 0);
    $this->db->like($keyfilter);   
    $this->db->limit($limit,$start);
    $this->db->order_by("cpuser.userID","DESC");
    $query = $this->db->get();
